...
*/
/**************************************************************************/
bool Adafruit_BMP3XX::begin_I2C(uint8_t addr, TwoWire *theWire, uint32_t frequency) {
  if (i2c_dev)
    delete i2c_dev;
  if (spi_dev)
    delete spi_dev;
  ...
  if (!i2c_dev->setSpeed(frequency)) {
    Serial.println("Failed to set I2C frequency!");
    return false;
  }
...
*/
/**************************************************************************/
bool Adafruit_BMP3XX::readPandT( float* press, float* temp) {
  bool res = performReading();
  *press = pressure;
  *temp = temperature;
  return res;
}
...
/**************************************************************************/
/*!
    @brief Performs a config outside of the performReading.

    @return True on success, False on failure
*/
bool Adafruit_BMP3XX::performConfig( uint8_t mode) {
  int8_t rslt;
...
bool Adafruit_BMP3XX::performReading(void) {
  ...
  //performConfig( BMP3_MODE_FORCED);
