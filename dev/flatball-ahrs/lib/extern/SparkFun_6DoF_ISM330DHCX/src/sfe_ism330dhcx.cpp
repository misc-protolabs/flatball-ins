...
// getRaw_acc_gyr()
//
// Retrieves raw register values for accelerometer and gyroscope data
//
//  Parameter    Description
//  ---------    -----------------------------
//  acc_buffer   Pointer to an array of 3 int16_t values to store the accelerometer data
//  gyr_buffer   Pointer to an array of 3 int16_t values to store the gyroscope data
bool QwDevISM330DHCX::getRaw_acc_gyr(sfe_ism_raw_data_t *acc_buf, sfe_ism_raw_data_t *gyr_buf)
{
	int16_t tmp_acc[3] = {0};
	int16_t tmp_gyr[3] = {0};
	int32_t ret_acc = ism330dhcx_acceleration_raw_get(&sfe_dev, tmp_acc);
	int32_t ret_gyr = ism330dhcx_angular_rate_raw_get(&sfe_dev, tmp_gyr);
	...
	acc_buf->xData = tmp_acc[0];
	acc_buf->yData = tmp_acc[1];
	acc_buf->zData = tmp_acc[2];

	gyr_buf->xData = tmp_gyr[0];
	gyr_buf->yData = tmp_gyr[1];
	gyr_buf->zData = tmp_gyr[2];
