...
bool SFE_MMC5983MA::readFieldsXYZ(uint32_t *x, uint32_t *y, uint32_t *z)
{
    ...
    if (success)
    {
        ...
        *x = (registerValues[0] << 10) | (registerValues[1] << 2) | (registerValues[6] >> 6);
        *y = (registerValues[2] << 10) | (registerValues[3] << 2) | ((registerValues[6] >> 4) & 0x03);
        *z = (registerValues[4] << 10) | (registerValues[5] << 2) | ((registerValues[6] >> 2) & 0x03);    }
    else
    {
        SAFE_CALLBACK(errorCallback, SF_MMC5983MA_ERROR::BUS_ERROR);
    }
